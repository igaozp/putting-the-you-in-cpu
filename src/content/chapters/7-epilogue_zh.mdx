...
首先，线程的栈是一个固定数量的内存，它被映射到虚拟内存中的某个高位。在大多数（虽然[不是全部](https://stackoverflow.com/a/664779)）架构上，栈指针从栈内存的顶部开始，并在递增时向下移动。物理内存不会为整个映射的栈空间预先分配；相反，使用按需分页来延迟分配内存，当到达栈的帧时。
